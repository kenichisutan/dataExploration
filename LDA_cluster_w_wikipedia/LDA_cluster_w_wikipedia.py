...
def load_and_process_data(paths, stop_words, column_name='content'):
    all_text_list = []
    ...
    for path in paths:
        if os.path.exists(path):
            ...
            all_text_list.extend(df[column_name].tolist())
        else:
...
def lda_on_clusters(raw_texts, clusters, name):
    ...
    with open(f'{name}_clusters.txt', 'w') as f:
        ...
        for cluster_id in sorted_clusters:
            ...
            print(f"\nCluster {cluster_id} (Total Articles: {cluster_texts_count}):\n")
            sample_texts = cluster_df[cluster_df['cluster'] == cluster_id]['text'].sample(min(3, cluster_texts_count), random_state=42).tolist()
            for text in sample_texts:
                f.write(f"{text}\n\n")
                print(f" - {text[:100]}...")

# Matching ABC articles to Wikipedia clusters using cosine similarity
def match_abc_to_wikipedia_clusters(abc_tfidf, wiki_tfidf, wiki_clusters):
    similarities = cosine_similarity(abc_tfidf, wiki_tfidf)
    matched_clusters = similarities.argmax(axis=1)
    matched_clusters = [wiki_clusters[idx] for idx in matched_clusters]  # Map to actual Wikipedia clusters
    return matched_clusters

def main():
    # Load and process Wikipedia dataset
    wiki_texts, wiki_raw_texts = load_and_process_data(
        ['../all_events_with_external_content_and_summary.csv'], stop_words, "content_summary")
    wiki_tfidf_matrix, wiki_vectorizer = extract_features(wiki_raw_texts, stop_words)
    wiki_clusters, wiki_km = apply_clustering(wiki_tfidf_matrix, num_clusters=10)

    # Check number of unique Wikipedia clusters
    print(f"Number of unique Wikipedia clusters: {len(set(wiki_clusters))}")

    # Load and process ABC dataset
    abc_texts, abc_raw_texts = load_and_process_data(
        ['/home/kenich/MultiLayrtET2_Project/Data/2_proccessed_data_and_analysis/data/selected_data/articles/ABC/ABC.csv'], stop_words)
    abc_tfidf_matrix, abc_vectorizer = extract_features(abc_raw_texts, stop_words)

    # Match ABC articles to Wikipedia clusters
    abc_clusters = match_abc_to_wikipedia_clusters(abc_tfidf_matrix, wiki_tfidf_matrix, wiki_clusters)

    # Check number of unique matched clusters
    print(f"Number of unique matched clusters: {len(set(abc_clusters))}")

    # Run LDA on matched clusters
    lda_on_clusters(abc_raw_texts, abc_clusters, 'ABC')
    silhouette_avg = silhouette_score(abc_tfidf_matrix, abc_clusters)
